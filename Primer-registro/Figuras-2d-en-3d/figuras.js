...
function createPac(gl)
{
    ...
    for(let angle=d2r(45);angle < d2r(315);angle+=d2r(1))
    {
        verts.push(0,0,0)
        verts.push(Math.cos(angle)*radius,
        Math.sin(angle)*radius,0)
        verts.push(Math.cos(angle+d2r(1))*radius,
        Math.sin(angle+d2r(1))*radius,0)
    }


    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    
    let pacman = {buffer:vertexBuffer, vertSize:3, nVerts:verts.length/3, primtype:gl.TRIANGLES};
    return pacman;   
}
