...
class triangle
{
    ...
    {
        ctx.beginPath()
        ctx.moveTo(this.vertex1[0], this.vertex1[0])
...
function drawLevels(points,steps)
{
    ...
    if (steps > 0) {
        ...
        drawLevels([points[0],newPoints[0],newPoints[0],newPoints[2]], steps - 1)
        drawLevels([points[1],newPoints[0],newPoints[0],newPoints[1]], steps - 1)  
        drawLevels([points[2],newPoints[1],newPoints[0],newPoints[2]], steps - 1)          
...
function getSliderVal(canvas,points,ctx)
{
    document.getElementById("slider").oninput = function(event)
    {
        ...
        document.getElementById("valSlider").innerHTML = "Recursion Level: " + event.target.value
